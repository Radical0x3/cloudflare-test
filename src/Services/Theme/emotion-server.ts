...
type CreateExtractCriticalToChunks = (html: string) => {
	html: string;
	styles: { key: string; ids: string[]; css: string | boolean }[];
};

function createExtractCriticalToChunks(cache: EmotionCache): CreateExtractCriticalToChunks {
...
function generateStyleTag(cssKey: string, ids: string, styles: string | boolean, nonceString: string): any {
	return `<style data-emotion="${cssKey} ${ids}"${nonceString}>${styles}</style>`;
}

function createConstructStyleTagsFromChunks(cache: EmotionCache, nonceString: string): any {
	return function (criticalData: ReturnType<ReturnType<typeof createExtractCriticalToChunks>>) {
		let styleTagsString = '';

		criticalData.styles.forEach((item: any) => {
...
export function createEmotionServer(cache: EmotionCache): {
	extractCriticalToChunks: CreateExtractCriticalToChunks;
	constructStyleTagsFromChunks: any;
} {
